...
const placeOrder = async (req, res) => {
  try {
    ...
    // Add this check before Order.create(...)
    const activeOrder = await Order.findOne({
      where: {
        customerId: req.user.id,
        status: { [require('sequelize').Op.ne]: 'delivered' },
      },
    });
    if (activeOrder) {
      return error(res, 'You already have an active order. Wait for it to be delivered before placing a new one.', 409);
    }
...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    return success(res, { order: updated }, `Order status updated to ${status}`);
  } catch (err) {
    return serverError(res, err.message);
  }
};
